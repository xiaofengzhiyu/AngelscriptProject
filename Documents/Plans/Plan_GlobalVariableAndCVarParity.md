...
- 当前仍未形成 `FConsoleCommand` 的专项测试闭环，也还没有把 global variable / `cvar` 能力面单独列为独立计划入口，因此后续 backlog 仍然容易被总计划淹没。
...
- [ ] **P2.1** 固化 `FConsoleVariable` 当前已对齐能力，并补上 testing engine 相关回归
